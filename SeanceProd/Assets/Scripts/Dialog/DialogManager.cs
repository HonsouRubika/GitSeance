	...
	public class DialogManager : MonoBehaviour
	{
		...
		public bool DisplayDialog(string txt)
		{
			...
			else
			{
				...
				if (_isAudioLinked && GameManager.AudioManager != null && GameManager.AudioManager._mjSource.isPlaying)
				{
					_timeBetweenSentences = GameManager.AudioManager._mjSource.clip.length + 2f;
				}
				else if (_isAudioLinked && GameManager.AudioManager == null)
				{
					Debug.LogError("AudioManager missing is Scene");
				}
				else if (_isAudioLinked && !GameManager.AudioManager._mjSource.isPlaying)
				{
